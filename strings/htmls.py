...
def filt_html_tags(htmlStr: str) -> str:
...
def filt_base64(htmlStr: str) -> str:
    """清除Base64编码的图片，给makemid存html时节省空间用"""
    return re.sub('<img src="data:image/png;base64,[^>]*>', "", htmlStr)


def filt_js(htmlStr: str) -> str:
    """清除JavaScript代码，给makemid存html时节省空间用"""
    return re.sub("<script[^<]*</script>", "", htmlStr)


def filt_wechat_tags(text: str) -> str:
    """
    微信文章特色标记处理
    """
...
# ['', 'swwww', '22ddd']
def split_by_tag(htm: str, tag: str = "<h1") -> List[str]:
    """输入：html长string
...
def extract_head_txt(txt: str) -> str:
...
if __name__ == "__main__":
    # 示例用法
    htmlText = "<p>这是一个<b>测试</b>文本</p>"
    print("原文本:", htmlText)
    print("清除HTML标签后:", filt_html_tags(htmlText))

    linksHtml = '<p>链接1<a href="http://example.com">示例</a>和链接2<a href="http://test.com">测试</a></p>'
    print("解析链接:", parse_links(linksHtml))
